...
pub async fn list_todos(Extension(pool): Extension<PgPool>, Extension(tera): Extension<Tera>, cookies: Cookies) -> Html<String> {
    match cookies.get("login") {
        Some(login) => {
            let is_doesnt_exist = sqlx::query!("SELECT COUNT(1) FROM accounts WHERE login = $1", login.value().to_string())
                .fetch_one(&pool)
                .await
                .unwrap();
